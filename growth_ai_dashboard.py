...
def generate_channel_data(days=30, seed=42):
    ...
    conversions = np.maximum(10, (sessions * rng.uniform(0.02, 0.07, size=days)).astype(int))
...
def generate_leads(n=200, seed=7):
    ...
    source = rng.choice(["Organic", "Paid Search", "Referral", "LinkedIn", "Direct"], size=n)
    industry = rng.choice(["Fintech", "EdTech", "HealthTech", "SaaS", "E-commerce"], size=n)
...
def normalize_leads_df(df):
    ...
    expected = [
        "company", "source", "industry", "visits", "pages_viewed",
        "time_on_site_sec", "downloaded_asset", "demo_requested", "email_open_rate"
    ]
    missing = [c for c in expected if c not in df.columns]
    if missing:
        raise ValueError(f"Leads CSV missing columns: {', '.join(missing)}")

    for col in ["visits", "pages_viewed", "time_on_site_sec", "downloaded_asset", "demo_requested", "email_open_rate"]:
        df[col] = pd.to_numeric(df[col], errors="coerce")
...
def get_ai_insights(channel_df, leads_df):
    ...
    best_source = high_or_hot["source"].value_counts().idxmax() if not high_or_hot.empty else "Organic"
    ...
    insights = [
        ...
        f"There are {hot_leads} hot leads right now. Prioritize founder-led or sales-led outreach to the top 10 within 24 hours.",
...
def build_exec_summary(channel_df, leads_df):
    ...
    hot_leads = int((leads_df["priority"] == "Hot").sum())
    best_source = leads_df.groupby("source")["lead_score"].mean().sort_values(ascending=False).index[0]
...
with right:
    ...
    st.bar_chart(leads_df["priority"].value_counts().sort_index())

    st.subheader("Average Lead Score by Source")
    st.bar_chart(leads_df.groupby("source")["lead_score"].mean().sort_values(ascending=False))
...
with f3:
    min_score = st.slider("Minimum lead score", 0, int(leads_df["lead_score"].max()), 80)
